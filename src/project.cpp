...
// Writes the binary representation of classfile into the given destination
// path.
...
void Project::remove_unused_methods()
{
    ...
    std::vector<Method> reachable_methods =
        this->method_call_graph(main_method);
...
std::vector<Method> Project::sibling_methods(const Method &m) const
{
    auto method_is_sibling = [&](const Method &o) -> bool {
        return m.method_type() == o.method_type() and
               m.method_name() == o.method_name() and
               class_is_descendant(o.class_file(), m.class_file());
    };
    ...
    for (const ClassFile &cf : this->m_files) {
        for (const Method &o : Method::all_from_classfile(cf)) {
            if (method_is_sibling(o)) {
                ...
                debug << m.format() << " and " << o.format()
                      << " are not sibling methods\n";
            }
        }
    }
...
bool Project::class_is_descendant(const ClassFile &down,
                                  const ClassFile &up) const
{
    if (down == up) {
        return true;
    }
    ...
    if (is_reachable(up)) {
        debug << down->class_name() << " is a descendant of "
              << up->class_name() << "\n";
        return true;
    } else {
        debug << down->class_name() << " is NOT a descendant of "
              << up->class_name() << "\n";
        return false;
    }
}
