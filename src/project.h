...
class Project
{
  ...
  public:
    ...
    void save_in_place(
        std::vector<std::experimental::filesystem::path> filenames) const;

    /// Returns all of the class files.
    std::vector<ClassFile> classfiles() const;

    /// Returns all of the methods which are reachable by a series of calls from
    /// the given method.
  ...
  private:
    ...
    bool class_is_descendant(const ClassFile &down, const ClassFile &up) const;
};
